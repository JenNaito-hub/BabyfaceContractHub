...
import type { Kenh, TrangThaiDon } from "./types";
...
/** Ánh xạ trạng thái trên sàn → trạng thái nội bộ. */
export function mapTrangThai(raw: string | null | undefined): TrangThaiDon {
  const s = String(raw ?? "").toLowerCase();
  if (!s) return "moi";
  if (/(huỷ|hủy|cancel)/.test(s)) return "huy";
  if (/(hoàn|return|refund)/.test(s)) return "hoan";
  if (/(hoàn thành|đã giao|delivered|completed)/.test(s)) return "hoan_thanh";
  if (/(đang giao|shipping|in transit|shipped|to ship|chờ giao)/.test(s)) return "dang_giao";
  if (/(đã xác nhận|confirm|awaiting|chờ lấy hàng|processing)/.test(s)) return "da_xac_nhan";
  return "moi";
}
